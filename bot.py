...
async def on_submit(self, interaction: discord.Interaction):
    ...
    # VALIDARE NUMĂR JUCĂTORI
    try:
        needed = int(self.jucatori.value.strip())
    except ValueError:
        return await interaction.response.send_message(
            "❌ Introdu un număr valid de jucători.",
            ephemeral=True
        )

    if needed < 1 or needed > 10:
        return await interaction.response.send_message(
            "❌ Numărul de jucători trebuie să fie între 1 și 10.",
            ephemeral=True
        )

    rank_text = self.rankuri.value.strip()
    ...
    await interaction.response.send_message(
        embed=embed,
        view=LFGView(needed, interaction.user)
    )
...
class CautaView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="cauta_ranked"
    )
    async def ranked(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer(ephemeral=True)
        await interaction.followup.send("Folosește comanda **/lfg**", ephemeral=True)

    @discord.ui.button(
        label="Unrated",
        style=discord.ButtonStyle.success,
        custom_id="cauta_unrated"
    )
    ...
    @discord.ui.button(
        ...
        custom_id="cauta_altele"
    )
    async def altele(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer(ephemeral=True)
        await interaction.followup.send("Folosește comanda **/premier**", ephemeral=True)
