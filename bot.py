...
class LFGView(discord.ui.View):
    ...
    async def join(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user in self.joined:
            return await interaction.response.send_message("❌ Deja înscris.", ephemeral=True)
        if len(self.joined) >= self.needed:
            return await interaction.response.send_message("❌ Echipa completă.", ephemeral=True)
...
class LFGModal(discord.ui.Modal):
    def __init__(self, mod):
        ...
        self.jucatori = discord.ui.TextInput(label="Câți jucători cauți?", placeholder="Ex: 4")
        self.rankuri = discord.ui.TextInput(label="Rank range", placeholder="Gold – Platinum")

        self.add_item(self.jucatori)
        self.add_item(self.rankuri)

    async def on_submit(self, interaction: discord.Interaction):
        needed = int(self.jucatori.value)

        embed = discord.Embed(
            title=f"{self.mod}: Looking for {needed} players!",
            color=0xff4655,
            timestamp=datetime.utcnow()
        )

        embed.add_field(name="🏆 Rank range", value=self.rankuri.value)
        embed.add_field(name="🎮 Mod", value=self.mod)
        embed.set_footer(
            text=interaction.user.display_name,
            icon_url=interaction.user.display_avatar.url
        )

        await interaction.response.send_message(embed=embed, view=LFGView(needed))
...
class CautaView(discord.ui.View):
    ...
    async def unrated(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(LFGModal("Unrated"))
    ...
    async def altele(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(LFGModal("Altele"))
